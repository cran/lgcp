...
lgcpPredictMultitypeSpatialPlusPars <- function(formulaList,
    ...
    else if (inherits(spatial.offsetList,"SpatialAtRisk")){
        for(i in 1:ntypes){
            spatial[[i]] <- spatial.offsetList
        }
    }
    else if (inherits(spatial.offsetList,"list")){
        for(i in 1:ntypes){
            if(!inherits(spatial.offsetList[[i]],"spatialAtRisk")){
	...
	if (!is.null(output.control$gridfunction) & inherits(output.control$gridfunction[1],"dump2dir")){
